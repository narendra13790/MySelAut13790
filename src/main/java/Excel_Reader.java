...
public class Excel_Reader
{
	...
	public Excel_Reader(String sExcelFilePath)
	{
	...
	/**
	 * true- test has to be executed false- test has to be skipped
	 * 
	 * @param testCaseName
	 *          - name of the test case
	 * @param xls
	 *          - object reference of the {@link Excel_Reader}
	 * @return -return true if testcase run mode set to Y( means yes)
	 */
	public boolean isExecutale(String testCaseName)
	{
		for (int rowNum = 2; rowNum <= getRowCount("Test Cases"); rowNum++)
		{
			if (getCellData("Test Cases", "TCID", rowNum).equals(testCaseName))
			{
				if (getCellData("Test Cases", "Runmode", rowNum).equals("Y"))
					return true;
				else
					return false;
			}
	 ...
	 * 
	 * @param sSheetname
	 *          -give the name for sheet
	 * @return returns true if sheet is created successfully else false
	 */
	public boolean createSheet(String sSheetname)
	{
		FileOutputStream fileOut;
		try
		{
			workbook.createSheet(sSheetname);
			fileOut = new FileOutputStream(sExcelFilePath);
	 ...
	 */
	public boolean removeSheet(String sSheetName)
	{
		int index = workbook.getSheetIndex(sSheetName);
		if (index == -1)
			return false;
	...
	// as parameter.
	public static String[][] getDataForTests(String sSheetName, String sTextToIdentifyStartRow, String sFileName)
	{
		// find the row num from which test starts
		Excel_Reader objXls = new Excel_Reader(sFileName);
		int testStartRowNum = 0;
		// find the row num from which test starts
		for (int iRowNum = 1; iRowNum <= objXls.getRowCount(sSheetName); iRowNum++)
		{
			if (objXls.getCellData(sSheetName, 0, iRowNum).equals(sTextToIdentifyStartRow))
			{
